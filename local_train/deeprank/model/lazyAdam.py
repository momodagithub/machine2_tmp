...
class LazyAdamOptimizer(optimizer.Optimizer):
  ...
  def _apply_sparse(self, grad, var):
      lr = (self._lr_t * math_ops.sqrt(1 - self._beta2_t) / (1 - self._beta1_t))
      # m_t = beta1 * m + (1 - beta1) * g_t
      m = self.get_slot(var, "m")
      m_scaled_g_values = grad.values * (1 - self._beta1_t)
      m_scaled = tf.gather(m, grad.indices) * self._beta1_t
      m_t = state_ops.scatter_update(m, grad.indices,
                                     m_scaled + m_scaled_g_values,
                                     use_locking=self._use_locking)
      m_tp = tf.gather(m_t, grad.indices)

      # v_t = beta2 * v + (1 - beta2) * (g_t * g_t)
      v = self.get_slot(var, "v")
      v_scaled_g_values = (grad.values * grad.values) * (1 - self._beta2_t)
      v_scaled = tf.gather(v, grad.indices) * self._beta2_t
      v_t = state_ops.scatter_update(v, grad.indices,
                                     v_scaled + v_scaled_g_values,
                                     use_locking=self._use_locking)
      v_tp = tf.gather(v_t, grad.indices)
